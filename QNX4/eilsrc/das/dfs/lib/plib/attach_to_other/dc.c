...
void DC_exitfunction(void) {
  ...
  if (oper_loop) {
	...
	if (ds_tid>0) {
	  pid_t pr;
	  dr_tok.n_rows = 0;
	  /* trigger data source with a null token */
	  if ((pr=qnx_proxy_a(ds_tid,(char *)&dr_tok,MSG_TOKEN_SZ,-1))!=-1) {
	    while (qnx_trig(pr) == -1)
	      if (errno==EINTR) continue;
	      else {
		msg(MSG_WARN,"Can't trigger proxy %d attached to task %d",
		    pr,ds_tid);
		ds_tid=-1;
		break;
	      }
	  } else {
	    msg(MSG_WARN,"Can't attach proxy to task %d",ds_tid);
	    ds_tid=-1;
	  }
	}
	break;
...
int DC_init(topology_type top, long node) {
  ...
  switch (dc_topology) {
  case STAR: setprio(getpid(),getprio(0)-1);
  case RING: /* locate name */
    ...
    setmx(&mlist[0], &dr_init, MSG_HDR_SZ);
    setmx(&mlist[1], &dbr_info, DBR_INFO_SZ);
    while (!breaksignal) {
      if ( !(oper_loop=!qnx_sndmx(ds_tid, 1, 2, &mlist, &mlist)) )
	if (errno == EINTR) continue;
	else {
	  ds_tid = -1;
	  msg(MSG_EXIT_ABNORM,"Error sending to data source task %d",ds_tid);
	}
      if (dr_init == DAS_BUSY && !DC_IS_STAR) continue;
      else break;
    }
    break;
  case BUS:
...
reply_type DC_process_msg() {
  ...
  if (dfs_who > 0) {
	...
	if (dfs_msg->u.drd.n_rows == 0 && dfs_who != ds_tid) {
	    dc_tok = dfs_who;
