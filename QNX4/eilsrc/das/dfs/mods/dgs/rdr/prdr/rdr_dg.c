...
int DG_get_data( token_type n_rows ) {
  ...
  for (mfcount = 0; mfcount < mfs; ) {
    ...
    if (curfd <= 0 && curfile <= tofile)
      if (getfilename( curfilename, dirname, rootname, curfile++,
		      filesperdir, 0))
	if ( (curfd = open( curfilename, O_RDONLY, 0))  != -1) {
	  if (lseek(curfd, curposition, SEEK_SET) == -1) {
	    msg(MSG_WARN,"Can't seek to position %d in %s",curposition,
		basename(curfilename));
	    nexttimepos=0;
	    CLOSEFILE;
	    continue;
	  }
	...
	if (j != sizeof(tstamp_type) || i != 2) {
	  if (j == -1 || i == -1)
	    msg(MSG_WARN,"Can't read timestamp from file %s: rest of file skipped",basename(curfilename));
	  ...
	  curposition = tell(curfd);
	  nexttimepos = (next_ts==LAST_STAMP) ? 0 : 
	    curposition + (next_ts * tmi(nbrow));
	  if (T.secs != dbr_info.t_stmp.secs || 
	      T.mfc_num != dbr_info.t_stmp.mfc_num) {
	    dbr_info.t_stmp=T;
	    DG_s_tstamp(&T);
	    return 1;
	  }
	...
	if (j != tmi(nbminf)) {
	  ...
	  default:
	  case -1: msg(MSG_WARN,"error reading file %s, rest of file skipped",basename(curfilename));
	  }
	}
	continue;
      }
...
reply_type DG_other(unsigned char *msg_ptr, pid_t sent_tid) {
  /* called when a message is recvd which is not handled by the distributor */
  ...
  case TIME_SET:
    switch( *(msg_ptr+1)) {
    case TIME_START:
      set_time(msg_ptr+2,starttime,&setfromfile,&setfrompos,
	       &setnexttimepos,&set_stamp,"Start");
      break;
    case TIME_END:
...
void set_time(char *m, time_t t, int *f, long *p,  long *n, tstamp_type *s,
	      char *h) {
  time_t timet, timet2;
  unsigned short mf;
  ...
  if (timet<starttime || timet>endtime) {
    msg(MSG_WARN,"Requested %s time: out of range: reset to Limit %s time",
	h,h);
    timet=t;
  }
  ...
  if ( (*f = findmf( timet, startfile, endfile, dirname, rootname,
		    filesperdir, p, n, s, &mf)) == -1 )
