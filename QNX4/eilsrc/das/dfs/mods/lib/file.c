...
time_t gettimeval( char *string, time_t usetime ) {
  ...
  /* Stupid year 2000 fix for the next ten years! */
  if (restime.tm_year < 10) restime.tm_year +=100;
...
return -1 on error.
*/
int findmf( time_t secs, int startfile, int endfile, char *dirname,
	   char *rootname, int fperd, long *pos, long *nextstamp,
	   tstamp_type *stamp, unsigned short *mfcounter ) {
