...
int  i;
	...
	if (DG_init_options(argc,argv) != 0)
		msg(MSG_EXIT_ABNORM,"Can't initialise as a DG");
	mt_dg = dbr_info.mod_type;

    /* initialise into DBR */
    if (DC_init_options(argc,argv) != 0) 
		msg(MSG_EXIT_ABNORM,"Can't initialise as a DC");
	mt_dc = dbr_info.mod_type;
	nt_dc = dbr_info.next_tid;
