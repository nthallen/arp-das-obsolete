...
void GW_dc_data(dbr_data_type *dr_data) {
...
void GW_dc_tstamp(tstamp_type *tstamp)  {
  if (DG_IS_STAR) {
    /* add to stamp list */
    llist *t;
...
void GW_dc_DASCmd(unsigned char type, unsigned char number) {
  if (DG_IS_STAR) {
    llist *c;
...
reply_type GW_dc_other(unsigned char *msg_ptr, int who) {
  if (DG_IS_STAR)
    switch (*msg_ptr) {
    case DCINIT:
      return(init_star_client(who));
      break;
    case DCTOKEN:
      if (*(msg_ptr+1)==UBYTE1_MAX) done_client(who);
      break;
    }
  return(DAS_NO_REPLY);
}
...
int send_cmd_or_stamp(llist *l, arr *a, unsigned long lseq) {
  if ( a->list_seen <= lseq && (l->seq==a->seq ||
				(l->seq > a->seq && (t_data_seq - a->seq) < 
				 dbr_info.nrowminf))) {
    if (l->hdr==DCDASCMD)
      GW_dg_s_dascmd(l->u.c.type,l->u.c.val);
    else GW_dg_s_tstamp(&l->u.s);
    a->seq=l->seq;
    a->list_seen++;
...
int send_data(token_type nrows, arr *a) {
  ...
  if (req_seq < t_data_seq) {
    ...
    GW_dg_s_data(circrows,bfr+atrow*tmi(nbrow),rows-circrows,bfr);
    a->seq += rows;
  } else GW_dg_s_data(0,0,0,0);
  return 1;
}
