...
void DC_data(dbr_data_type *dr_data) {
  ...
  if (logging && dr_data->n_rows) {
    ...
    if (logged_bytes < maxfilesize)
      rows_space = ((maxfilesize-logged_bytes-rows_left_in_mf*
		     tmi(nbrow))/(dbr_info.nrowminf*tmi(nbrow)))*
		       dbr_info.nrowminf + rows_left_in_mf;
    ...
    if (fwrite(dr_data->data, 1, bytes_to_log, fp)!=bytes_to_log)
      msg(MSG_EXIT_ABNORM,"error writing %d bytes to %s",bytes_to_log,
	  basename(name));
    ...
    if (dr_data->n_rows != rows_to_log) {
      if ( !(remaining_data=malloc(2+(dr_data->n_rows-rows_to_log)*
				   tmi(nbrow))))
	msg(MSG_EXIT_ABNORM,"Can't allocate memory");
      remaining_data->n_rows = dr_data->n_rows - rows_to_log;
      memmove(remaining_data->data, dr_data->data+bytes_to_log, 
	      (dr_data->n_rows - rows_to_log)*tmi(nbrow));
...
void DC_tstamp( tstamp_type *tstamp )  {
  ...
  if (!minfrow) {
	...
	SWITCHFILE;
      /* write last timestamp's trailing bytes; num of rows between them */
      if (logged_bytes) {
	if ( fseek(fp,ftell(fp)-(rows_since_stamp * tmi(nbrow) + 2),
		   SEEK_SET)!=0)
	  msg(MSG_EXIT_ABNORM,"Can't seek in %s",basename(name));
	if (fwrite( &rows_since_stamp, 2, 1, fp)!=1)
	  msg(MSG_EXIT_ABNORM,"Can't write next tstamp indicator in %s",
	      basename(name));
	...
	msg(MSG_EXIT_ABNORM,"Can't write tstamp follower in %s",
	    basename(name));
...
reply_type DC_other(unsigned char *msg_ptr, pid_t sent_tid) {
  /* nut 'n honey */
  return DAS_UNKN;
}
