...
/* defines */
#define HDR "ctrl"
#define OPT_MINE "r:d:Rp:P"
#define MAX_MSG_TYP    MAX_GLOBMSG  /* Maximum number of different message types */
#define MAX_IDS        50	    /* Maximum number of active valid ids at a time */
#define STARTER_PROG "strtr"
...
void handle_ccreg (pid_t id_to_relay_to, ccreg_type *ccreg_msg);
void handle_death (pid_t id);
...
void release_relays (int down_id);
void remove_from_quit (int remove_id);
...
int loop_var;
	...
	table[loop_var].id=0;
	if (table[loop_var].ptr) free(table[loop_var].ptr);
    }
}

void handle_ccreg (pid_t new_tid, ccreg_type *ccreg_msg) {
...
*/
	...
	if  ( !((ccreg_msg->min_dasc == 0) && (ccreg_msg->max_dasc == 0))) {
	    no_dascmds = 0;
	    for (check_it = ccreg_msg->min_dasc; check_it <= ccreg_msg->max_dasc; check_it++)
	...
	if  ( (!((ccreg_msg->min_msg == 0) && (ccreg_msg->max_msg == 0))) && (ccreg_state == DAS_OK)) {
	    no_msgs = 0;
	    for (check_it = ccreg_msg->min_msg; check_it <= ccreg_msg->max_msg; check_it++)
		if (relay_msgs[check_it] != 0) {
		    ccreg_state = DAS_BUSY;
		    break;
		}
        }  else no_msgs = 1;
	if (table[hsh].ptr) ccreg_state = DAS_BUSY;

	if (ccreg_state == DAS_OK) {
	    /* Check quit state */
	    if (ccreg_msg->how_to_quit > MAX_QUIT_TYPE || ccreg_msg->how_to_die > MAX_QUIT_TYPE)
		...
		if (!strt_tid && (ccreg_msg->how_to_die==TASK_RESTART || ccreg_msg->how_to_die==DAS_RESTART || ccreg_msg->how_to_die==REBOOT))
		    if (!(strt_tid=spawnlp(P_NOWAIT,starter_prog,starter_prog,NULL)) )
			msg(MSG_EXIT_ABNORM,"Can't start %s",starter_prog);
		if (no_dascmds != 1)
		    for (check_it = ccreg_msg->min_dasc; check_it <= ccreg_msg->max_dasc; check_it++)
...
void reply_msg (pid_t reply_to_tid, reply_type message_byte) {
	...
	if (hsh==-1)
	    ...
	    msg(MSG_WARN,"replied %s to %s",table[hsh].task_name);
...
void cmd_ctrl_loop (void) {
	...
	if ((tid_that_sent=Receive(0,recv_msg,MAX_MSG_SIZE))==-1)
		...
		case DEATH:
		    ...
		    if ( (hsh=get_hash(tid_that_sent))<HASH_SIZE)
			if (table[hsh].id) handle_death(tid_that_sent);
		    break;
		case DASCMD:
		    relay_dascmd (tid_that_sent, (dasc_msg_type *) recv_msg);
		    break;
		case CCReg_MSG:
		    hsh=get_hash(tid_that_sent);
		    handle_ccreg (tid_that_sent, (ccreg_type *) recv_msg);
		    break;
		default:
    ...
    if (table[hsh].ptr)
		...
		case TASK_RESTART:
			...
			if (strt_tid) {
			    ...
			    if (replycode !=DAS_OK)
				msg(MSG_WARN,"Bad response from %s",starter_prog);
			}
			release_relays(tid);
			break;
		case DAS_RESTART:
			...
			exit(0);
			break;
		case SHUTDOWN: shut_down(); break;
		case REBOOT:
			msg(MSG,"rebooting");
			sprintf(fullname,"%c",REBOOT);
			if (strt_tid) {
			    if ( (Send(strt_tid,fullname,&replycode,1,sizeof(reply_type)))==-1)
				msg(MSG_WARN,"Can't send to %s",starter_prog);
			    if (replycode !=DAS_OK)
				msg(MSG_WARN,"Bad response from %s",starter_prog);
			}
...
void release_relays (pid_t down_id) {
    int loop_var;       
    for (loop_var = 0; loop_var < MAX_MSG_TYP; loop_var++) {
    ...
    remove_from_quit (down_id);
    return;
}

void remove_from_quit (pid_t remove_id) {
int quit_idx;
int id_found;
    id_found = 0;
    if (table[hsh].ptr) {
