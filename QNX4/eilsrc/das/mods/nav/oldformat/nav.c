...
void main(int argc, char **argv) {
    ...
    while (!terminated) {
		...
		if (!fr) {
			...
			fr = 1;
			buf[0] = !(FRAME_CHAR);
			msg(MSG_DEBUG,"bad frame: invalid framing chars");
			bad_frames++;
			continue;
		    }
		}
		...
		/* data sanity check */
		sanity1 = MAKEWORD(6); /* lattitude */
		sanity2 = MAKEWORD(20); /* altitude */
		if (sanity1 > 16384 || sanity1 < -16384 || sanity2 > -5582) {
			bad_frames++;
			msg(MSG_DEBUG,"bad frame: data sanity check failed");			
			continue;
		}
		...
		msg(MSG_DBG(1),"time deviation: %d",wd);
		nav_params[0] = wd;
		wd = MAKEWORD(6); /* lattitude */
		msg(MSG_DBG(1),"lattitude: %d",wd);
		nav_params[1] = wd;
		wd = MAKEWORD(8); /* longitude */
		msg(MSG_DBG(1),"longitude: %d",wd);
		nav_params[2] = wd;
		wd = MAKEWORD(20); /* altitude */
		msg(MSG_DBG(1),"altitude: %d",wd);
		nav_params[4] = wd;
		wd = MAKEWORD(22); /* true air speed */
		msg(MSG_DBG(1),"true air speed: %u",wd);
		nav_params[3] = wd;
