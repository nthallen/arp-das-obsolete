	...
	if (buffer[0]=='/' && buffer[1]=='*') commcount++;
		...
		switch (commcount) {
			...
			/* form: lines, cols, pos_y, pos_x */
				if (sscanf(buffer,"%*s %d %d %d %d", &fhdr.rows,&fhdr.cols,&fhdr.pos_y,&fhdr.pos_x)!=4) return(0);
				posy=(posy<0)?fhdr.pos_y:posy;
				posx=(posx<0)?fhdr.pos_x:posx;
				inwin=newwin(fhdr.rows,fhdr.cols,posy,posx);
				inwin->fill_attr=0;
				wclear(inwin);
			break;
			case 2:
			...
			/* fields: number, line, col, width of field, attribute code, string */
				if (top) {
					...
					*top=f;
				    ...
				    if (sscanf(buffer,"%*s %d %d %d %d %d %d \"%[^\"\n]",&f->num,&f->line,&f->col,&f->width,&f->length,&f->attrcode,buf) < 6) return(0);
