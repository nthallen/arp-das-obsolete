...
int fld_win_out(char * filename,WINDOW *codescr,struct fldinfo *top,char **attrtypes, int numattr, int lines, int cols, int posy, int posx) {
int i,j,k,c, strt=0;
int fattr;
FILE *fp;
struct fldinfo *p;
...
p=top;
while (p) {
	fprintf(fp,"#FIELD# %u %u %u %u %u %d \"",p->num, p->line,p->col,p->width,p->length,p->attrcode);
	fprintf(fp,"%s\"\n",p->txt);
	p=p->fldnext;
}
...
int fld_win_in(char *filename, WINDOW *win, struct fldinfo **top,
 int *numflds, char **attrtypes, int posy, int posx, struct hdr *fldhdr) {
	...
	if (buffer[0]=='/' && buffer[1]=='*') commcount++;
		...
		switch (commcount) {
			...
			/* form: lines, cols, pos_y, pos_x */
				if (sscanf(buffer,"%*s %d %d %d %d", &fldhdr->rows,&fldhdr->cols,&fldhdr->pos_y,&fldhdr->pos_x)!=4) return(0);
				posy=(posy<0)?fldhdr->pos_y:posy;
				posx=(posx<0)?fldhdr->pos_x:posx;
				inwin=newwin(fldhdr->rows,fldhdr->cols,posy,posx);
				inwin->fill_attr=0;
				wclear(inwin);
			break;
			case 2:
			...
			/* fields: number, line, col, width of field, attribute code, string */
				...
				if (sscanf(buffer,"%*s %d %d %d %d %d %d \"%[^\"\n]",&f->num,&f->line,&f->col,&f->width,&f->length,&f->attrcode,buf)!=7) return(0);
...
int fld_num(struct fldinfo *f, int cy, int cx, int ey, int ex, struct fldinfo **info) {
struct fldinfo *p;

p=f;
while (p)
    if ( (cy<=p->line && cx<=p->col && (ey==-1 || (ey>=p->line+p->length-1 && ex>=p->col+p->width-1)))
	|| ( cy>=p->line+p->length-1 && cx>=p->col+p->width-1 && (ex==-1 || (ex<=p->col && ey<=p->line))) )  {
			if (info) *info=p;
			return(p->num);
	} else  p=p->fldnext;
...
int fld_add(struct fldinfo **fldtop,int num,char *txt,int line, int col, int width,
