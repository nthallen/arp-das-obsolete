...
main(int argc, char **argv) {
  ...
  shdr.nattrs=0;
  if (initscr()==ERR) msg(MSG_FATAL,"Cannot curses initialise");
  scrollok(stdscr,FALSE);
  if ( (stdcodescr=newwin(LINES,COLS,0,0)) == NULL)
    msg(MSG_FATAL,"Cannot create new curses window");

  /* first check for .scr or .fld extensions */
  if (strstr(argv[optind],".scr")) {
   if (!scr_win_in(argv[optind], stdcodescr, 0, -1, -1, &shdr))
     msg(MSG_FATAL,"Error creating screen from scr file");
  }
  else if (strstr(argv[optind],".fld")) {
    if (!fld_win_in(argv[optind], stdcodescr, 0, 0, 0, -1, -1, &shdr))
      msg(MSG_FATAL,"Error creating screen from fld file");
  }
  else
    if ( !scr_win_in(argv[optind], stdcodescr, 0, -1, -1, &shdr) )
      if ( !fld_win_in(argv[optind], stdcodescr, 0, 0, 0, -1, -1, &shdr) )
	if ( !scr_win_in(extfilename(argv[optind],"scr",space2),
			 stdcodescr, 0, -1, -1, &shdr) )
	  if (!fld_win_in(extfilename(argv[optind],"fld",space2),
		     stdcodescr, 0, 0, 0, -1, -1, &shdr))
	    msg(MSG_FATAL,"Error creating screen from fld file");

  if (shdr.nattrs) {
    if (argc > optind+1) {
      if (!init_attrs(argv[optind+1],attributes,shdr.nattrs))
	if (!init_attrs(extfilename(argv[optind+1],"cfg",space2),
			attributes,shdr.nattrs))
	  msg(MSG_FAIL,"error reading cfg of %s",argv[optind+1]);
    }
    else init_attrs(extfilename(argv[optind],"cfg",space2),
		    attributes,shdr.nattrs);
