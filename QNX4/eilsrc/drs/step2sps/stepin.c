...
int read_hdr(FILE *fp, char *ss_name, int *niv, int *nv, int *nsph,
  ...
  *nauxv=0;
  ...
  default:
    cmderr("Only FFI 1001, 1010, 1020, 2010 and 2110 Formats accepted");
    return(-1);  	
  }
  ...
  *bd = (double) mktime(&timeptr);
  ...
  if (ffi == 2010) {
    if (*nxwhich>0) {
      ...
      if (dr_calc_cmd_io("Which constant bounded independent variable value (1,2..)? [1] ",buf,1)) strcpy(buf,"1");
  ...
  if (ss_error(ss = ss_create(ss_name, (nmon || ffi==2110) ? 
			      SPT_NON_MONOTONIC : SPT_INCREASING,
			      *nv + (ffi==2110 ? 2 : 1), 1))) {
  ...
  if (set_scale_miss_name(fp, ss, *nv, sf, mv, ffi==2110 ? 2 : 1)) return(-1);

  /* read number auxillary variables */
  if (ffi == 1020 || ffi == 1010 || ffi == 2010 || ffi == 2110) {
    read_line(fp, buf);
    buf[69] = '\0';
    dr_calc_cmd_line(buf, NULL);
  ...
  if (auxing) {
    ...
    if (set_scale_miss_name(fp, ass, *nauxv, asf, amv, 1)) return(-1);
  }
  else if (*nauxv)
    for (i=0;i<*nauxv+2;i++)
      swlw_ln(fp);
...
int set_scale_miss_name(FILE *fp, sps_ptr ss, int nv, double *sf, double *mv,
  ...
  for (i = 0; i < nv; i++) {
    ...
    /* read names */
    read_line(fp, buf);
    if (ss_set_column_title(ss, start+i, buf)) {
      cmderr("Error %d setting column %d title", snafu, start+i);
      return(-1);
    }
    if (ss_set_column_format(ss, start+i, "%9.2e")) {
      cmderr("Error %d setting column %d format", snafu, start+i);
      return(-1);
    }
    buf[69] = '\0';
    dr_calc_cmd_line(buf, NULL);
...
int read_value(FILE *fp, sps_ptr ss, int i, double T, double *sf,
               double *mv, int *missed, double delta, double bd, int nmon) {
  double V;
...
void stepin(char *filename, char *ss_name) {
  ...
  for (;;) {
	...
	for (i = 0; i < nauxv; i++)
	  /* read auxillary vars */   	      
	  if (read_value(fp, ass, i, hack, asf, amv, amissed,
			 0, bd, nmon)) break;
      }
      else swlw_ln(fp);
    /* figure out loops */
    ...
    /* read primary vars */
    for (T = hack, j = 0; j < nsph; j++, T += dx[0]) {
      for (n = 0; n < a; n++) {
	for (i = 0; i < b; i++) {
	  if (ffi==2110 && i==0)
	    /* read bounded independent var, treat as primary var */
	    if (read_value(fp, ss, -1, T, 0, 0, 0, 0, bd, ffi==2110 ? 1:nmon))
	      break;
	  if (nxwhich==(i+1) || ffi==2110) {
	    if (read_value(fp, ss, ffi==2110 ? i : n, T, sf, mv, missed,
			 delta, bd, ffi==2110 ? 1 : nmon))
	      break;
	  }
	  else fscanf(fp,"%*lf");
	}
