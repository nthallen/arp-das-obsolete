...
void msg_init(char *hdr, char *fn, int verbose, char *targ, char *oarg,
	...
	if (targ && strlen(targ)) {
	    ...
	    if ( (memo_tid=qnx_name_locate(nd, LOCAL_SYMNAME(memo_name),0,0))==-1)
			msg(MSG_EXIT_ABNORM,"Can't find symbolic name for %s on node %ld",memo_name,nd);
	}
#endif

	if (fn && strlen(fn))
	    if ( (msg_fd = open(fn,O_WRONLY | O_CREAT | O_APPEND | O_SYNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH)) == -1)
