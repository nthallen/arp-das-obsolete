...
#define OZONE_BASE 0x680
#define H2O_BASE 0x690
#define CNTBASE H2O_BASE

void read_counters(unsigned char *dbuf) {
  (*((unsigned int *) (&dbuf[0]))) = read_subbus(0, CNTBASE + 0);
  dbuf[2] = read_subbus(0, CNTBASE + 2) & 0xFF;
  (*((unsigned int *) (&dbuf[3]))) = read_subbus(0, CNTBASE + 4);
  dbuf[5] = read_subbus(0, CNTBASE + 6) & 0xFF;
  (*((unsigned int *) (&dbuf[6]))) = read_subbus(0, CNTBASE + 8);
  (*((unsigned int *) (&dbuf[8]))) = read_subbus(0, CNTBASE + 0xA);
}
